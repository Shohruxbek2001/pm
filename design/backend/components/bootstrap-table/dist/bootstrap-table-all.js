...
!function ($) {
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        $.each(this.options.columns, function (i, column) {
            ...
            if (!column.visible) {
                // Fix #229. Default Sort neworder is wrong
    ...
    BootstrapTable.prototype.onSort = function (event) {
        ...
        this.$header.add(this.$header_).find('span.neworder').remove();
            ...
            $sortModal.off('click', '.btn-primary').on('click', '.btn-primary', function() {
                ...
                that.options.sortPriority = $.map($rows, function(row) {
                    var $row = $(row),
                        ...
                        order = $row.find('.multi-sort-neworder').val();
    ...
    BootstrapTable.prototype.addLevel = function(index, sortPriority) {
        ...
        $sortModal.find('tbody')
            .append($('<tr>')
                ...
                .append($('<td>').append($('<select class="form-control multi-sort-neworder">')))
            );

        var $multiSortName = $sortModal.find('.multi-sort-name').last(),
            $multiSortOrder = $sortModal.find('.multi-sort-neworder').last();
