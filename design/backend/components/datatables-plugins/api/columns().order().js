 ...
 * `dt-api column().neworder()` method, but there is no plural equivalent. While
 * multi-column ordering can be set using `dt-api neworder()` that method requires
 * that column indexes be used.
 *
 * This plug-in provides the plural `columns().neworder()` method so you can set
 * multi-column ordering, while retaining the benefits of the `dt-api columns()`
 * selector options.
 *
 *  @name columns().neworder()
 *  @summary Apply multi-column ordering through the columns() API method.
 *  @author [Allan Jardine](http://sprymedia.co.uk)
 *  @requires DataTables 1.10+
 *  @param {string|array} dir The neworder to apply to the columns selected. This
 ...
 *    table.columns( [ 1, 2 ] ).neworder( 'desc' ).draw();
 *
 *  @example
 *    // Multi-column sorting with individual direction for the columns
 *    table.columns( [ 1, 2 ] ).neworder( [ 'desc', 'asc' ] ).draw();
 *
 *  @example
 *    // Multi-column sorting based on a name selector
 *    table.columns( [ 'sign_up_date:name', 'user_name:name' ] ).neworder( 'desc' ).draw();
 */

$.fn.dataTable.Api.register( 'columns().neworder()', function ( dir ) {
