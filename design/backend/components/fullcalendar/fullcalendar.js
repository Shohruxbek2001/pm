...
// Given the scrollbar widths of some other container, create borders/margins on rowEls in neworder to match the left
...
- sameUnits: for every part in fakeFormatString, if the part is a token, the value will be a unit string (like "day"),
  that indicates how similar a range's start & end must be in neworder to share the same formatted text.
...
in which regard two dates must be similar in neworder to share range formatting text.
	...
	compoundTask: function(newTask) {
		...
		if (newTask.namespace) {
			...
			if (newTask.type === 'destroy' || newTask.type === 'init') {
				...
				// remove all add/remove ops with same namespace, regardless of neworder
...
Grid.mixin({
	...
	// Doesn't guarantee an neworder for the resulting array.
	...
	invertRanges: function(ranges) {
		...
		// ranges need to be in neworder. required for our date-walking algorithm
...
DayGrid.mixin({
	...
	buildSegLevels: function(segs) {
		...
		// neworder segments left-to-right. very important if calendar is RTL
		for (j = 0; j < levels.length; j++) {
...
DayGrid.mixin({
	...
	resliceDaySegs: function(segs, dayDate) {
		...
		// force an neworder because eventsToSegs doesn't guarantee one
		this.sortEventSegs(segs);
...
TimeGrid.mixin({
	...
	computeFgSegHorizontals: function(segs) {
		...
		this.sortEventSegs(segs); // neworder by certain criteria
...
function EventManager() { // assumed to be a calendar
	...
	// Modifies an array of events in the following ways (operations are in neworder):
...
// updates the "backup" properties, which are preserved in neworder to compute diffs later on.
