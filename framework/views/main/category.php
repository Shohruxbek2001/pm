...
<main class="container">
    ...
    <div class="row">
    <section class="col-md-12 catalog menu-bar">
        <ul class="filter"><?php
            $catalog = \app\models\CategoryType::find()->all();
            ?>
            <?php
            foreach ($catalog as $index) {
                ?>
                <li class="filter__item">
                <a href="" class="filter__link"><?= $index->category_type_ru ?></a>
                <div class="sub-menu-1" style="width: 220px" >
                    <ul><?php
                        $category = \app\models\Catalog::find()->all();
                        ?>
                        <?php foreach ($category as $index1) if ($index->category_type_id == $index1->category_type_id) { ?>
                            <li >
        ...
        <div class="col-md-10 catalog__container">
            <ul class="catalog__list good__list">
                <!--Коробека для товара-->
                <!-- Sale товар  -->
                <?php foreach ($models as $index) { ?>
                <div class="col-md-4">
                    <li class="item good__item" tabindex="-1" style="width:280px;height:414px">
                        <!--Иконка для знака sale-->
                        <?php if ($index->category_id == 1) { ?>
                            <svg width="37" height="35" class="item__sale">
                                <use xlink:href="#sale"></use>
                            </svg><?php } ?>
                        <?php if ($index->category_id == 2) { ?>
                        ...
                        <div class="item__title-container">
                            ...
                            <small class="item__mark"><?= $index->evaluation ?>
                                <svg width="15" height="15" class="item__star">
                                    <use xlink:href="#starv2"><?= $index->no_discount_ru ?></use>
                                </svg>
                            </small>
                        </div>
                        <!-- Описания товара -->
                        <p class="item__des row"><?= substr($index->details_ru,0,20) ?>...</p>
            ...
            <?php foreach ($product
                ...
                <?php if ($index->category_id == 1) { ?>
                    <li class="item sale__item" tabindex="-1">
                        ...
                        <!-- Описания товара -->
                        <p class="item__des"><?= substr($index->details_ru,0,20)?>...</p>
                        <div class="item__bottom--sale">
                            <!-- Старая цена -->
                            <del class="sale__old-price"><?= $index->no_discount_ru ?></del>
                            <!-- Контейнер для кнопки и цены -->
